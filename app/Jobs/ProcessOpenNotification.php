...
class ProcessOpenNotification implements ShouldQueue
{
    ...
    public function handle(GetIncommingNotificationType $typeProcessor): void
    {
        match ($typeProcessor->handle($this->notification)) {
            ...
            // No delay: the zaak is already gone in the zaaksysteem, so there is
            // nothing to wait for and nothing to collapse into one job.
            OpenNotificationType::ZaakDestroyed => ZaakDestroyNotificationReceived::dispatch($this->notification),
