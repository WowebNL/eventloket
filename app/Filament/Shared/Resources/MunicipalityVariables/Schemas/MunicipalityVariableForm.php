                ...
                Section::make(__('Meldingsvraag informatie'))
                    ...
                    ->schema([
                        TextEntry::make('report_question_info')
                            ->hiddenLabel()
                            ->state(__('Een meldingsvraag wordt getoond in het aanvraagformulier om te bepalen of voor de aanvraag een vergunning aangevraagd moet worden of dat een melding voldoende is. Er bestaat al een basisset van standaard meldingsvragen, de vragen die hier worden aangemaakt gelden alleen voor de gemeente :name en worden toegevoegd aan de basisset. De vraag moet te beantwoorden zijn met "ja" of "nee". Indien het antwoord "nee" is zal dan zal er een vervolgvraag worden gesteld om te bepalen of de aanvraag een melding is. Indien het antwoord op de vraag "ja" is dan zal de aanvraag worden gekenmerkt als vergunningsplichtig. Vul bij de waarde de vraag in welke te beantwoorden is met "ja" of "nee".', ['name' => $municipality->name ?? 'onbekend']))
                            ->visible(fn (Get $get) => $get('type') === MunicipalityVariableType::ReportQuestion)
