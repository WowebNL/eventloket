...
- **GlobaleLocatie.** De locatie van het evenement wordt als zaakobject van type "overige" (objectTypeOverige "GlobaleLocatie") meegestuurd. Heeft de aanvraag een of meer BAG-adressen, dan gaan die adressen mee (bijvoorbeeld "Coriovallumstraat 32, 6411CD Heerlen"), bij meerdere adressen komma-gescheiden. Een adres zegt een behandelaar meer dan een zelfgekozen locatienaam. Heeft de aanvraag geen adres (een buitenevenement of een route), dan gaan de locatienamen uit het formulier mee. Voor dat type moet de identificatie onder `objectIdentificatie.overigeData` staan; een kale `objectIdentificatie.naam` wordt door zowel Open Zaak als OneGround geweigerd met een 400. Hier speelt wel een verschil in vorm. De ZGW-standaard typeert `overigeData` als een vrij-vorm object, dus voor Open Zaak stuurt Eventloket `{"naam": "..."}`. OneGround wijkt af en verwacht (en toont) `overigeData` als een kale tekst. Staat het vinkje **Dit is een OneGround koppeling** aan, dan stuurt Eventloket de locatienamen daarom als kale tekst in plaats van als object.
...
- **Organisator.** De organisator die Eventloket bij een doorkomstzaak toont (in de zakenlijst, de zaakdetails en de export) komt uit de aanvraag zelf, via de hoofdzaak in Eventloket. Die naam werd eerder uit de rol in het bronsysteem afgeleid en bleef daardoor leeg bij een systeem dat de identificatie van een rol niet teruggeeft, zoals OneGround.
