 ...
 *
 * A run that does succeed has to produce a zaakobject the ZGW API accepts:
 * ObjectAdres requires `identificatie` (the BAG nummeraanduiding id) and
 * expects the street name in gorOpenbareRuimteNaam, so a missing
 * identificatie is rejected with a 400.
 */
...
test('an adres zaakobject carries the BAG nummeraanduiding as identificatie and the street name', function () {
    ...
    $locationService->shouldReceive('forBackgroundWork')->andReturnSelf();
    $locationService->shouldReceive('lastRequestWasUnreachable')->andReturnFalse();
